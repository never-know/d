...
Min.event.bind('loginsubmit','click', function(){	  
	...
	if( sindex == 0 ){
		...
		minAjax({
			url:'http://passport.' + site_domain + '/login.html', 
			type:'POST', 
			data:{
			...
			success: function(data){
				if(data.status == 1 ){
					...
					if(data.status == -1){
						...
						var a = _$('login-code'),
							code_url = 'http://util.' + site_domain + '/captcha/get.html?type=login&v=';
...
function code_check(){
	...
	current_code = code;
	JSONP.get( 'http://util.' + site_domain + '/captcha/check.html', {code:code,type:'login'}, function(data){

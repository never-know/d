...
/**********tabbar***newadd*oldremove****/
+function(t){"use strict";var e="weui_bar_item_on",n=function(n){var i=t(n);if(!i.hasClass(e)){var a=i.attr("href");if(/^#/.test(a)){i.parent().find("."+e).removeClass(e),i.addClass(e);var r=i.parents(".weui_tab").find(".weui_tab_bd");r.find(".weui_tab_bd_item_active").removeClass("weui_tab_bd_item_active"),t(a).addClass("weui_tab_bd_item_active")}}};t.showTab=n,t(document).on("click",".weui_tabbar_item",function(i){var a=t(i.currentTarget),r=a.attr("href");a.hasClass(e)||(/^#/.test(r)&&(i.preventDefault(),n(a)))||(a.parent().find("."+e).removeClass(e),a.addClass(e))})}($);
/*fastclick*/
...
// date format
 Date.prototype.Format = function (fmt) { //author: meizz  (new Date()).Format("yyyy-MM-dd hh:mm:ss.S")
    var o = {
...
var page_load_loading= false,page_load_current_page=1,page_load_total_page=2;

function page_load(url,template, reset) {
 
if (reset == true) {
page_load_loading = false;page_load_current_page=1;page_load_total_page=2;
return;
}

$('.weui_tab_bd').infinite(250).on("infinite", function() {
console.log(page_load_loading +'   ' +page_load_current_page +'   ' + page_load_total_page);
 
        if(page_load_loading) return;
        if(page_load_total_page <= page_load_current_page ) return;
        page_load_loading = true;
		$.ajax({
			url:url, 
			type:'GET', 
			data: (($('#list_form').length>0)?$('#list_form').serialize():{page:page_load_current_page+1}),
			success: function(data){
				if (data.statusCode == 1 ) {
					if (data.body.list.length > 0) {
						$("#list_loaded").append($(data.body.list).map(template ).get().join(' '));
					}
					page_load_current_page = data.body.page.current_page;
					page_load_total_page = data.body.page.total_page;
					if($('#next_page').length>0) {
						$('#next_page').val(page_load_current_page+1);
					}
					if (page_load_total_page == page_load_current_page)　{

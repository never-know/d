 
<div class="content-wrapper">	
	<div style="width:950px;float:left;">
		<div class="success">
		
			<h3> <i class="iconfont">&#xe634;</i>商品已成功加入购物车！</h3>
			<span id="flashBuy" style="display:none">商品数量有限，请您尽快下单并付款！</span>
			<a class="to-cart"  href="http://cart.annqi.com/view.html" >去购物车结算</a>
			<em class="ml10">您还可以 <a href="javascript:history.back();">继续购物</a></em>
				 
		</div>
		<div class="plist-recommend plist-recommend-bottom">
			<div class="recommend-title">
				 购买该商品同时还购买了 
			</div>
			
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
		
		
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
	
		
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
			
		</div>
		
		
		<div class="plist-recommend plist-recommend-bottom">
			<div class="recommend-title">
				 你可能想购买以下产品 
			</div>
			
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
		
		
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
	
		
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a>
				</dt>
				<dd>
				<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
				<span>￥200.00</span>
				<em>立即购买</em>
				</dd>
			</dl>
			
		</div>
		
		
		
		
	</div>
	<div class="mycart-leftbar">
		<div class="mycart">
			<p  class="title">我的购物车</p>
			 <span  class="title">刚刚加入购物车的商品</span>
			<dl>
				<dt><a href="http://details.annqi.com/item/123456123456.html" class="sn"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a></dt>
				<dd>
					<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
					<span>￥200.00</span>
					<em>&times;1</em>
				</dd>
			</dl>
			<span class="title"> 购物车中以前的商品</span>
			<dl>
				<dt> <a href="http://details.annqi.com/item/123456123456.html" class="sn"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a></dt>
				<dd>
					
					<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
					<span>￥200.00</span>
					<em>&times;1</em>
				</dd>
			</dl>
			<dl>
				<dt> <a href="http://details.annqi.com/item/123456123456.html" class="sn"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a></dt>
				<dd>
					
					<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
					<span>￥200.00</span>
					<em>&times;1</em>
				</dd>
			</dl>
			<dl>
				<dt> <a href="http://details.annqi.com/item/123456123456.html" class="sn"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a></dt>
				<dd>
					
					<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
					<span>￥200.00</span>
					<em>&times;1</em>
				</dd>
			</dl>
			<dl>
				<dt> <a href="http://details.annqi.com/item/123456123456.html" class="sn"><img src="http://cdn.annqi.com/public/images/product/2.jpg" alt=""  > </a></dt>
				<dd>
					
					<p><a href="">内蒙古科尔沁 休闲零食 风干牛肉干原味400g</a></p>
					<span>￥200.00</span>
					<em>&times;1</em>
				</dd>
			</dl>
			
			<div class="summary"  >
				<p>共 <em>5</em> 件商品</p>
				<p>总共金额  <strong  >￥2200.00</strong></p>
				
			</div>
		</div>
		<div class="ck">去购物车结算</div>
	</div>
</div>
	


	
	
	
	 